...
export class ListenerFactoryService {
  ...
  async createListener(
    ...
    if (this.activeListeners.has(listenerKey)) {
      ...
      this.logger.log(
        `Returning existing listener for chain ${sdk.chainId} with strategy ${strategy}`,
      );
    ...
    try {
      ...
      this.logger.error(
        `Failed to create listener for chain ${sdk.chainId}:`,
        error,
      );
      throw error;
    }
  ...
  async startListener(
    ...
    try {
      ...
      this.logger.error(
        `Failed to start listener for chain ${sdk.chainId}:`,
        error,
      );
      throw error;
    }
  ...
  async stopListener(chainId: number, strategy: EventStrategy): Promise<void> {
    ...
    if (!listener) {
      this.logger.warn(
        `No listener found for chain ${chainId} with strategy ${strategy}`,
      );
      return;
    }
  ...
  getActiveListeners(): Array<{ key: string; isRunning: boolean }> {
    return Array.from(this.activeListeners.entries()).map(
      ([key, listener]) => ({
        key,
