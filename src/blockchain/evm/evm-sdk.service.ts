...
export class EvmSdkService implements IBlockchainSDK {
    ...
    try {
      ...
      if (config.wsUrl) {
        try {
          ...
          this.logger.warn(`Failed to connect WebSocket for chain ${this.chainId}:`, error);
          this.wsProvider = null;
        }
      }

      this.logger.log(`Connected to ${config.name} (Chain ID: ${this.chainId})`);
    } catch (error) {
  ...
  createListener(strategy: EventStrategy): IBlockchainListener {
    ...
    switch (strategy) {
      case EventStrategy.WEBSOCKET:
        if (!this.wsProvider) {
          throw new Error(`WebSocket provider not available for chain ${this.chainId}`);
        }
      ...
      case EventStrategy.BLOCK_SCAN:
        if (!this.provider) {
          throw new Error(`HTTP provider not available for chain ${this.chainId}`);
        }
      ...
      case EventStrategy.HYBRID:
        ...
        throw new Error(`No provider available for hybrid strategy on chain ${this.chainId}`);
  ...
  async getLatestBlock(): Promise<number> {
    ...
    try {
      ...
      this.logger.error(`Failed to get latest block for chain ${this.chainId}:`, error);
      throw error;
    }
  }
